# Scripture References

This appendix tracks all biblical references cited throughout the book, organized by book and chapter. Each reference links to the chapter(s) where it is discussed.

---

## Old Testament

### Genesis
- **Genesis 1-3** — Creation, Life, and the Presence of God
  - See: [Chapter 1 — Creation, Life, and the Presence of God](../part-01/chapter-01.md)
- **Genesis 4** — Abel's sacrifice and blood
  - See: [Chapter 9 — Covenant Cleanness, Divine Presence, and Relational Judgment](../part-03/chapter-09.md)

### Exodus
- **Exodus 12** — Passover
  - See: [Chapter 4 — Passover: Salvation Through Death, Not Punishment](../part-02/chapter-04.md)

### Leviticus
- *(References to be added as chapters are written)*

### Numbers
- *(References to be added as chapters are written)*

### Deuteronomy
- *(References to be added as chapters are written)*

### Psalms
- *(References to be added as chapters are written)*

### Isaiah
- *(References to be added as chapters are written)*

### Jeremiah
- **Jeremiah 31** — New Covenant
  - See: [Chapter 10 — New Covenant or Renewed Covenant?](../part-04/chapter-10.md)

### Ezekiel
- *(References to be added as chapters are written)*

---

## New Testament

### Matthew
- *(References to be added as chapters are written)*

### Mark
- *(References to be added as chapters are written)*

### Luke
- **Luke 22** — The Cup, the Curse
  - See: [Chapter 12 — The Jealous Husband](../part-04/chapter-12.md)

### John
- *(References to be added as chapters are written)*

### Acts
- *(References to be added as chapters are written)*

### Romans
- **Romans 7** — Covenant Death and Release from Bondage
  - See: [Chapter 11 — Covenant Death and Release from Bondage (Romans 7)](../part-04/chapter-11.md)
- *(Additional Romans references to be added as chapters are written)*

### 1 Corinthians
- *(References to be added as chapters are written)*

### 2 Corinthians
- *(References to be added as chapters are written)*

### Galatians
- *(References to be added as chapters are written)*

### Ephesians
- *(References to be added as chapters are written)*

### Philippians
- *(References to be added as chapters are written)*

### Colossians
- *(References to be added as chapters are written)*

### Hebrews
- *(References to be added as chapters are written)*

### 1 Peter
- *(References to be added as chapters are written)*

### 1 John
- *(References to be added as chapters are written)*

### Revelation
- *(References to be added as chapters are written)*

---

## How to Use This Appendix

As you write chapters and cite scripture:

1. **Add the reference** under the appropriate book
2. **Include a brief description** of how it's used
3. **Link to the chapter(s)** where it appears
4. **Group related references** together for easier navigation

### Format Example:
```markdown
- **Book Chapter:Verse** — Brief description
  - See: [Chapter X — Title](../part-XX/chapter-XX.md)
  - See: [Chapter Y — Title](../part-YY/chapter-YY.md) *(if discussed in multiple chapters)*
```

---

*This appendix will be expanded as the manuscript develops. Consider organizing by:*
- *Theological themes (atonement, covenant, participation, etc.)*
- *Chronological order within each book*
- *Cross-references to related passages*
